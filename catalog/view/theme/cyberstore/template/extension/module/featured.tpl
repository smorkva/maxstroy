					...
					<div class="caption">
						...
						<?php } ?>
								...
								<div class="actions">
									...
									<?php } else { ?>
										<div class="cart"><button class="btn btn-general" type="button" onclick="cart.add('<?php echo $product['product_id']; ?>','featured', get_cart_quantity('<?php echo $product['product_id']; ?>','.carousel_numb_featured<?php echo $gen_carousel_featured;?>'));"><?php if($change_text_cart_button_out_of_stock ==1 && $product['product_quantity'] <= 0) { ?><i class="fa fa-lock" aria-hidden="true"></i> <span><?php echo $disable_cart_button_text; ?></span><?php } else { ?><i class="fa fa-shopping-basket"></i> <span><?php echo $button_cart; ?></span><?php } ?></button></div>
									<?php } ?>

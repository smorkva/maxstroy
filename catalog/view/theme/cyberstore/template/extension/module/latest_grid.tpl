...
<div class="container-modules latest_gv latest_grid<?php echo $module;?>">
  <?php foreach ($products as $product) { ?>
   <div class="item no-slider <?php echo $class_mob_row;?> col-sm-6 col-md-4 col-lg-1-5">
      ...
      <div class="image" style="height:127px !important">	
		...
		<?php if($show_special_timer_module =='1' && $product['special']){ ?>
			<div class="action-timer">
				<div id="countdown-latestgrid-<?php echo $module;?>-<?php echo $product['product_id'];?>"></div>		
			</div>
<script>
$(function(){
	var note = $('#note');
	...
	$('#countdown-latestgrid-<?php echo $module;?>-<?php echo $product['product_id'];?>').countdown({
		timestamp	: ts,
	  ...
	  <div class="caption">
		...
		<?php if(isset($config_additional_settings_newstore['quantity_btn_module']) && ($config_additional_settings_newstore['quantity_btn_module'] =='1')){?>
			<div class="quantity_plus_minus">
				<input type="text" class="quantity-num form-control input-number-quantity<?php echo $product['product_id'];?>" name="quantity" <?php if(isset($config_additional_settings_newstore['quantity_multiple']) && ($config_additional_settings_newstore['quantity_multiple'] == '1') && $product['minimum'] > 1){?>disabled<?php } ?> id="input_quantity_mod_latest_grid<?php echo $module;?><?php echo $product['product_id'];?>" onkeyup="validate_quantity(this,'<?php echo $product['minimum']; ?>')" oninput="recalc_quantity(<?php echo $product['product_id'];?>,<?php echo $product['minimum']; ?>, <?php echo $product['price_no_format'];?>,'<?php echo $product['special_no_format'];?>','.latest_grid<?php echo $module;?>','latest_gv');" value="<?php echo $product['minimum']; ?>">
				<span class="quantity-btn">
					<span class="add-up add-action fa fa-angle-up" onclick="btnplus_cat_price_latest_gv<?php echo $product['product_id'];?>('<?php if(isset($config_additional_settings_newstore['quantity_multiple']) && ($config_additional_settings_newstore['quantity_multiple'] == '1')){?><?php echo $product['minimum']; ?><?php } else { ?>1<?php } ?>');"></span>
					<span class="add-down add-action fa fa-angle-down" onclick="btnminus_cat_price_latest_gv<?php echo $product['product_id'];?>('<?php if(isset($config_additional_settings_newstore['quantity_multiple']) && ($config_additional_settings_newstore['quantity_multiple'] == '1')){?><?php echo $product['minimum']; ?><?php } else { ?>1<?php } ?>');"></span>
				</span>
			</div>
			<script>
			...
			<?php } ?>
				function btnminus_cat_price_latest_gv<?php echo $product['product_id'];?>(minimum){
					var $input = $('#input_quantity_mod_latest_grid<?php echo $module;?><?php echo $product['product_id'];?>');
					var count = parseInt($input.val()) - parseInt(minimum);
					count = count < parseInt(<?php echo $product['minimum']; ?>) ? parseInt(<?php echo $product['minimum']; ?>) : count;
					$input.val(count);
					$input.change();										
					recalc_quantity(<?php echo $product['product_id'];?>,count,<?php echo $product['price_no_format'];?>,'<?php echo $product['special_no_format'];?>','.latest_grid<?php echo $module;?>','latest_gv');						
				}
				function btnplus_cat_price_latest_gv<?php echo $product['product_id'];?>(minimum){
					var $input = $('#input_quantity_mod_latest_grid<?php echo $module;?><?php echo $product['product_id'];?>');
					var count = parseInt($input.val()) + parseInt(minimum);
					$input.val(count);
					$input.change();
  ...
  <?php if(isset($status_showmore) && $status_showmore !=0){?>
	<?php if($nextPage){?>
		<div class="showmore-latest<?php echo $module;?> box-showmore"><span class="box-ajax-load"><span data-nextpage="<?php echo $nextPage;?>" class="latest-ajax-load<?php echo $module;?>"><i class="fa fa-repeat" aria-hidden="true"></i> <?php echo $text_showmore;?></span></span></div>
	<?php } ?>
	<?php } ?>
  </div>
<?php if(isset($status_showmore) && $status_showmore !=0){?>
<script>
$(document).on('click', '.latest-ajax-load<?php echo $module;?>', function () {
	$.ajax({
		...
		dataType: 'html',
		beforeSend: function() {
			$('.showmore-latest<?php echo $module;?> i.fa-repeat').addClass('fa-spin');
		},
		complete: function() {
			$('.showmore-latest<?php echo $module;?> i.fa-repeat').removeClass('fa-spin');
		},	
		success:function(data){
			$data = $(data);
			var $products = $data.find('.latest_grid<?php echo $module;?> > div.item');
			...
			$('.latest_grid<?php echo $module;?>').append($products);
			$('.showmore-latest<?php echo $module;?>').replaceWith($data.find('.showmore-latest<?php echo $module;?>'));
		}
	});
})
</script>  

...
//			$json['redirect'] = $this->url->link('checkout/cart');
	...
	public function shipping_address($render = true, &$data  = array()) {
		...
		if ($render !== false)
		{
			...
			$this->response->setOutput($this->load->view('checkout/onepcheckoutshipping_address', $data));		
	...
	public function payment_address($render = true, &$data  = array()) {
		...
		if ($render !== false)	
		{
			 $this->response->setOutput($this->load->view('checkout/onepcheckoutpayment_address', $data));
		}	
  	}
  	...
  	public function payment_method($render = true, &$data = array()) {
		...
		$payment_address = array('country_id' => 0, 'zone_id' => 0, 'firstname' => '', 'lastname' => '', 'company' => '', 'address_1' => '');
		
		$new_s = isset($this->request->post['shipping_address']) && $this->request->post['shipping_address'] == 'new' ? 1 : 0;
		$new_p = isset($this->request->post['payment_address']) && $this->request->post['payment_address'] == 'new' ? 1 : 0;
		
		$new = $new_s ? $new_s : $new_p;
		
		if(!$this->customer->isLogged() || $new || isset($this->request->post['register'])) {
			$this->session->data['payment_address']['firstname'] = isset($this->request->post['firstname']) ? $this->request->post['firstname'] : '';
			$this->session->data['payment_address']['lastname'] = isset($this->request->post['firstname']) ? $this->request->post['lastname'] : '';
			...
			$this->session->data['payment_address']['shipping_city'] = isset($this->request->post['city']) ? $this->request->post['city'] : '';
			$this->session->data['payment_address']['country_id'] = isset($this->request->post['country_id']) ? $this->request->post['country_id'] : '';
			$this->session->data['payment_address']['shipping_country_id'] = isset($this->request->post['country_id']) ? $this->request->post['country_id'] : '';
			$this->session->data['payment_address']['zone_id'] = isset($this->request->post['zone_id']) ? $this->request->post['zone_id'] : '';
			$this->session->data['payment_address']['zone_country_id'] = isset($this->request->post['zone_id']) ? $this->request->post['zone_id'] : '';				
		...
		$existing_s = isset($this->request->post['shipping_address']) && $this->request->post['shipping_address'] == 'existing' ? 1 : 0;
		$existing_p = isset($this->request->post['payment_address']) && $this->request->post['payment_address'] == 'existing' ? 1 : 0;
		
		$existing = $existing_s ? $existing_s : $existing_p;
		...
		if($this->customer->isLogged() && $existing) {
			...
			$payment_address = $payment_address ? $payment_address : $shipping_address;
		}
	...
	private function sendNotify($order_data) {
		$dimms = [
			1 => "шт",
			7 => "кг",
			2 => "меш",
		...
		$message = "Нове замовлення з сайта\r\n";
		$message .= "<b>$lastname $firstname ($telephone)</b>\r\n";
		foreach($order_data["products"] as $product) {
			$name = $product["name"];
			$quantity = $product["quantity"];
			$dimmension = isset($dimms[$product["dimmention_id"]])
				? $dimms[$product["dimmention_id"]]
				: "шт";
			$message .= "    <i>$name $quantity$dimmension</i>\r\n";
		}

		$message .= "Всього " . $order_data["total"] . "грн";
		if(isset($order_data["comment"])){
			$message .= "\r\n<u>".$order_data["comment"]."</u>";
		}

		$ch = curl_init();

		curl_setopt($ch, CURLOPT_URL,"https://bot.maxstroy.in.ua/push.php");
			...
			if (isset($_POST) && !empty($_POST))
			{
				...
				foreach ($this->cart->getProducts() as $product) {
					...
					$order_data['products'][] = array(
						...
						'dimmention_id' => $product['length_class_id'],
					);
				}

...
class ControllerExtensionModuleCyberEditorplus extends Controller {
	...
	if(isset($this->request->post['route_mod']) && ($this->request->post['route_mod'] == 'product/category') || (isset($this->request->post['route_mod']) && ($this->request->post['route_mod'] == 'product/search'))){
		if(isset($this->session->data['token'])){
			if($data['user_logged']) { 
				if($this->request->post['route_mod'] == 'product/search'){
					$data['seo_category_id'] = 0;
				} else {
		...
		/*Product load*/
		if(isset($this->request->post['route_mod']) && ($this->request->post['route_mod'] == 'product/product')){
			$json = array();
			if(isset($this->session->data['token'])){

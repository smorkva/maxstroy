...
class ModelCatalogProductQuick extends Model {
	...
	public function getProductName($product_id) {
		...
		$row = $this->db->query("SELECT name FROM " . DB_PREFIX . "product_description WHERE product_id = '" . (int) $product_id . "' AND language_id = '" . (int)$selected_language . "'");
	...
	public function changeName($product_id, $name){
		...
		if ($this->user->hasPermission('modify', 'catalog/product_quick')) {
			$query = $this->db->query("UPDATE " . DB_PREFIX . "product_description SET name='".$this->db->escape($name)."' WHERE product_id = '" . (int)$product_id . "' AND language_id='" . (int)$selected_language . "'");
		}
		$query = $this->db->query("SELECT name FROM " . DB_PREFIX . "product_description WHERE product_id = '" . (int)$product_id . "' AND language_id= '" . (int)$selected_language . "'");
	...
	public function saveEditDescription($product_id,$user_add, $meta_title,$product_name,$meta_keyword,$meta_description,$description,$tag){
		$count_row = $this->db->query("SELECT COUNT(product_id) AS countdesc FROM `".DB_PREFIX."edit_description` WHERE product_id = '". (int)$product_id ."'");
		if($count_row->row['countdesc'] >= 10){
			$query_edit_desc = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_description WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified  ");
			$query = $this->db->query("UPDATE " . DB_PREFIX . "edit_description SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "',seo_title = '" . $this->db->escape($meta_title)  . "', product_name = '". $this->db->escape($product_name) ."',  meta_keyword = '". $this->db->escape($meta_keyword) ."', meta_description = '". $this->db->escape($meta_description) ."', description = '". $this->db->escape($description) ."', tag = '". $this->db->escape($tag) ."',  date_modified = NOW() WHERE date_modified = '". $this->db->escape($query_edit_desc->rows['0']['date_modified']) ."' ");
		} else {
			$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_description SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "',seo_title = '" . $this->db->escape($meta_title)  . "', product_name = '". $this->db->escape($product_name) ."', meta_keyword = '". $this->db->escape($meta_keyword) ."', meta_description = '". $this->db->escape($meta_description) ."', description = '". $this->db->escape($description) ."', tag = '". $this->db->escape($tag) ."', date_modified = NOW()");
		}
	}
	...
	public function getUserGroups($user_g_id) {
		$sql = "SELECT `user_group_id`, `status` FROM " . DB_PREFIX . "user_group where user_group_id = '". (int)$user_g_id ."'";	
 ...
 public function getCategoriesThis($product_id) {
			...
			foreach ($id_product_new as $new_id){
				...
				foreach($query->rows as $product_add_new){
					if($product_add_new['sku'] == ''){
						$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_price_product SET product_id = '" . (int)$product_add_new['product_id']  . "', product_name = '" . $this->db->escape($product_add_new['name']) . "', product_model = '" . $this->db->escape($product_add_new['model']) . "', product_sku = '" . $this->db->escape($product_add_new['model']) . "', quantity = '" . (int)$product_add_new['quantity'] . "', date_modified = NOW()");	
					} else {
						$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_price_product SET product_id = '" . (int)$product_add_new['product_id']  . "', product_name = '" . $this->db->escape($product_add_new['name']) . "', product_model = '" . $this->db->escape($product_add_new['model']) . "', product_sku = '" . $this->db->escape($product_add_new['sku']) . "', quantity = '" . (int)$product_add_new['quantity'] . "', date_modified = NOW()");	
					}
				}
	...
	$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_manual_price WHERE product_id = '" . (int)$product_id . "'");
		...
		foreach ($query->rows as $sel_pr_db){
			if(isset($sel_pr_db['product_id']) ||$sel_pr_db['product_id'] == $product_id){
				$this->db->query("DELETE FROM ". DB_PREFIX ."edit_manual_price WHERE product_id = '" . (int)$product_id . "'");
			}
		}
		
			$this->db->query("UPDATE " . DB_PREFIX . "product SET price = '" . (float)$data['price'] . "' WHERE product_id = '". (int)$product_id . "'");
		if(!empty($price_old)) { 
			$this->db->query("INSERT " . DB_PREFIX . "edit_manual_price SET product_id = '". (int)$product_id ."', manual_price = '" . (float)$price_old . "', date_end = '". $this->db->escape($data['date_end']) ."'");	
		} else {	
			$this->db->query("INSERT " . DB_PREFIX . "edit_manual_price SET product_id = '". (int)$product_id ."', manual_price = '" . (float)$edit_price . "', date_end = '". $this->db->escape($data['date_end']) ."'");		
		}
		...
		foreach($special_select->rows as $special){
			if($special){
				$this->db->query("INSERT " . DB_PREFIX . "edit_manual_price_save_special SET product_id = '". (int)$product_id ."',
				price = '" . $special['price'] . "',
	...
	public function JoinProductSku($product_merge, $product_additional, $user_add){
			...
			WHERE p.product_id = '". (int)$product_merge ."'");	
			...
			WHERE p.product_id = '". (int)$product_additional ."'");
			
			$general_product = $query_product_general->row;
			$additional_product = $query_product_additional->row;
			
			$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_username_join_product SET product_id = '" . (int)$product_merge  . "',
			...
			product_additional_price = '" . $this->db->escape($additional_product['price'])  . "',
			user_name = '" . $this->db->escape($user_add)  . "', date_join = NOW()");
			
			
			if(!empty($query_product_general->row)) {
			
			$query = $this->db->query("SELECT * FROM ". DB_PREFIX ."edit_join_sku WHERE product_id = '". (int)$additional_product['product_id'] ."'");
				...
				if(!empty($query->rows)){
					...
					$query = $this->db->query("UPDATE " . DB_PREFIX . "suppler_sku_description SET sku_id = '" . (int)$general_product['sku_id'] . "' WHERE sku_id = '". (int)$additional_product['sku_id'] ."'");
					
					$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_join_sku SET 
					product_id = '" . (int)$general_product['product_id']  . "',
					...
					price = '" . $this->db->escape($additional_product['price']) . "',
					bprice = '" . $this->db->escape($additional_product['bprice']) . "',
					quantity = '" . (int)$additional_product['quantity'] . "',
					date_modified = NOW()");
	...
	public function ChangesGeneralProduct($sku_product_additional){	
					...
					$query_addi_pr_array = $this->db->query("SELECT * FROM ". DB_PREFIX ."edit_join_sku WHERE product_sku = '" . $this->db->escape($sku_product_additional) . "'");
					$query_addi_pr = $query_addi_pr_array->row;
					
					/*Вытянул товар который сейчас основной*/
					$delete_join_sku_suppler_array = $this->db->query("SELECT * FROM ". DB_PREFIX ."edit_price_product WHERE product_id = '" . (int)$query_addi_pr['product_id'] . "'");	
					$delete_join_sku_suppler_array = $delete_join_sku_suppler_array->row;
					

					/*Удалил записи sku (разъединил)*/
					$this->db->query("DELETE FROM ". DB_PREFIX ."suppler_sku_description WHERE sku = '" . $this->db->escape($delete_join_sku_suppler_array['product_sku']) . "'");
					
					/*Обновил основной товар в edit_price_product!*/
					$query = $this->db->query("UPDATE " . DB_PREFIX . "edit_price_product SET  product_name = '" . $this->db->escape($query_addi_pr['product_name']) . "',
					product_model = '" . $this->db->escape($query_addi_pr['product_model']) . "',
					product_sku = '" . $this->db->escape($query_addi_pr['product_sku']) . "',
					quantity = '" . (int)$query_addi_pr['quantity'] . "',
					date_modified = NOW() WHERE product_id = '" . (int)$query_addi_pr['product_id']  . "'");
					...
					quantity = '" . (int)$query_addi_pr['quantity'] . "',
					price = '" . (float)$query_addi_pr['price'] . "' WHERE product_id = '" . (int)$query_addi_pr['product_id']  . "'");	
						
					/*Меняем Цену закупки Закупки !!! */
					$query = $this->db->query("SHOW TABLES LIKE '".DB_PREFIX ."suppler_base_price'");
						if($query->num_rows > 0){
							$bprice = $this->db->query("SELECT bprice FROM  " . DB_PREFIX . "suppler_base_price WHERE product_id = '". (int)$query_addi_pr['product_id'] ."'");
							if($bprice->num_rows > 0){
								$query = $this->db->query("UPDATE " . DB_PREFIX . "suppler_base_price SET  bprice = '" . (float)$query_addi_pr['bprice'] . "' WHERE product_id = '" . (int)$query_addi_pr['product_id']  . "'");
							}
						}
	...
	public function ChangeDescription($product_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_description WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified DESC");
		return $query->rows;
	}
	
	public function ChangeSpecial($product_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_special WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified DESC");
		return $query->rows;
	}
	
	public function SaveChangedSpecial($special_changes, $product_id, $user_add){
		$count_row = $this->db->query("SELECT COUNT(product_id) AS countspecial FROM ".DB_PREFIX."edit_special WHERE product_id = '". (int)$product_id ."'");
		if($count_row->row['countspecial'] >= 10){
			$query_edit_special = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_special WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified  ");
			$query = $this->db->query("UPDATE " . DB_PREFIX . "edit_special SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', special_changes = '". $this->db->escape($special_changes) ."',  date_modified = NOW() WHERE date_modified = '". $this->db->escape($query_edit_special->rows['0']['date_modified']) ."' ");
		} else {
			$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_special SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', special_changes = '". $this->db->escape($special_changes) ."', date_modified = NOW()");
		}
	}
	public function SaveChangedDiscount($discount_changes, $product_id, $user_add){
		$count_row = $this->db->query("SELECT COUNT(product_id) AS countdiscount FROM ".DB_PREFIX."edit_discount WHERE product_id = '". (int)$product_id ."'");
		if($count_row->row['countdiscount'] >= 10){
			$query_edit_discount = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_discount WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified  ");
			$query = $this->db->query("UPDATE " . DB_PREFIX . "edit_discount SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', discount_changes = '". $this->db->escape($discount_changes) ."',  date_modified = NOW() WHERE date_modified = '". $this->db->escape($query_edit_discount->rows['0']['date_modified']) ."' ");
		} else {
			$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_discount SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', discount_changes = '". $this->db->escape($discount_changes) ."', date_modified = NOW()");
		}
	}
	public function ChangeDiscount($product_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_discount WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified DESC");
		return $query->rows;
	}
	/*CATEGORY*/	
	public function ChangeCategory($product_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_category WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified DESC");
		return $query->rows;
	}
	public function SaveChangedCategory($manufacturer_id, $product_category, $product_id, $user_add){
		$count_row = $this->db->query("SELECT COUNT(product_id) AS countcategory FROM `".DB_PREFIX."edit_category` WHERE product_id = '". (int)$product_id ."'");
		if($count_row->row['countcategory'] >= 10){
			$query_edit_category = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_category WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified  ");
			$query = $this->db->query("UPDATE " . DB_PREFIX . "edit_category SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', manufacture = '". $this->db->escape($manufacturer_id) ."', subcategory = '". $this->db->escape($product_category) ."',  date_modified = NOW() WHERE date_modified = '". $this->db->escape($query_edit_category->rows['0']['date_modified']) ."' ");
		} else {
			$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_category SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', manufacture = '". $this->db->escape($manufacturer_id) ."', subcategory = '". $this->db->escape($product_category) ."',  date_modified = NOW()");
		}
	}
	/**/	
	/*IMAGE */
	public function ChangeImage($product_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_image WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified DESC");
		return $query->rows;
	}
	public function SaveChangedImage($general_image, $product_id,$additional_image,$user_add,$sort_addtitional_image){
		$count_row = $this->db->query("SELECT COUNT(product_id) AS countimage FROM ".DB_PREFIX."edit_image WHERE product_id = '". (int)$product_id ."'");
		if($count_row->row['countimage'] >= 10){
			$query_edit_image = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_image WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified  ");
			$query = $this->db->query("UPDATE " . DB_PREFIX . "edit_image SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', general_image = '". $this->db->escape($general_image) ."', additional_image = '". $this->db->escape($additional_image) ."', sort_addtitional_image = '". $this->db->escape($sort_addtitional_image) ."',  date_modified = NOW() WHERE date_modified = '". $this->db->escape($query_edit_image->rows['0']['date_modified']) ."' ");
		} else {
			$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_image SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', general_image = '". $this->db->escape($general_image) ."',additional_image = '". $this->db->escape($additional_image) ."', sort_addtitional_image = '". $this->db->escape($sort_addtitional_image) ."',  date_modified = NOW()");
		}
	}
	...
	public function SaveChangesImageUrl($general_image, $product_id,$additional_image,$user_add){
		$count_row = $this->db->query("SELECT COUNT(product_id) AS countimageurl FROM ".DB_PREFIX."edit_image_url WHERE product_id = '". (int)$product_id ."'");
		if($count_row->row['countimageurl'] >= 10){
			$query_edit_image = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_image_url WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified  ");
			$query = $this->db->query("UPDATE " . DB_PREFIX . "edit_image_url SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', general_image = '". $this->db->escape($general_image) ."', additional_image = '". $this->db->escape($additional_image) ."', date_modified = NOW() WHERE date_modified = '". $this->db->escape($query_edit_image->rows['0']['date_modified']) ."' ");
		} else {
			$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_image_url SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', general_image = '". $this->db->escape($general_image) ."',additional_image = '". $this->db->escape($additional_image) ."', date_modified = NOW()");
		}
	}
	public function SaveGeneralImageUrl($filepath, $product_id){
		$this->db->query("UPDATE " . DB_PREFIX . "product SET image = '" . $this->db->escape($filepath) . "' WHERE product_id = '" . (int)$product_id . "'");
	}
	public function SaveAdditionalImageUrl($filepath, $product_id){
		$this->db->query("INSERT INTO " . DB_PREFIX . "product_image SET product_id = '" . (int)$product_id . "', image = '" . $this->db->escape($filepath) . "'");
	}
	public function ChangeImageUrl($product_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_image_url WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified DESC");
		return $query->rows;
	}
	/*AND IMAGE URL*/
	...
	public function SaveChangesImageGoogle($general_image_google, $product_id,$additional_image_google,$user_add){
		$count_row = $this->db->query("SELECT COUNT(product_id) AS countimage_google FROM ".DB_PREFIX."edit_image_google WHERE product_id = '". (int)$product_id ."'");
			if($count_row->row['countimage_google'] >= 10){
				$query_edit_image = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_image_google WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified  ");
				$query = $this->db->query("UPDATE " . DB_PREFIX . "edit_image_google SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', general_image = '". $this->db->escape($general_image_google) ."', additional_image = '". $this->db->escape($additional_image_google) ."', date_modified = NOW() WHERE date_modified = '". $this->db->escape($query_edit_image->rows['0']['date_modified']) ."' ");
			} else {
				$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_image_google SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', general_image = '". $this->db->escape($general_image_google) ."',additional_image = '". $this->db->escape($additional_image_google) ."', date_modified = NOW()");
			}
	}
	
	public function ChangeImageGoogle($product_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_image_google WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified DESC");
		return $query->rows;
	}
	/*AND IMAGE GOOGLE*/
	public function SaveChangesRelatedProduct($product_id,$user_add, $related_changes){
		$count_row = $this->db->query("SELECT COUNT(product_id) AS countrelated FROM ".DB_PREFIX."edit_related WHERE product_id = '". (int)$product_id ."'");
		if($count_row->row['countrelated'] >= 10){
			$query_edit_related = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_related WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified  ");
			$query = $this->db->query("UPDATE " . DB_PREFIX . "edit_related SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', related_changes = '". $this->db->escape($related_changes) ."', date_modified = NOW() WHERE date_modified = '". $this->db->escape($query_edit_related->rows['0']['date_modified']) ."' ");
		} else {
			$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_related SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', related_changes = '". $this->db->escape($related_changes) ."', date_modified = NOW()");
		}
	}
	
	public function ChangeRelatedProduct($product_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_related WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified DESC");
		return $query->rows;
	}
	...
	public function SaveChangeDataProduct($product_id, $user_add, $model_product, $sku_product, $upc_product, $location_product, $tax_class_product,$quantity_product, $minimum_product,$subtract_product, $stock_status_product,$length_class_product,$weight_product,$weight_class_product,$lwh_product){
		$count_row = $this->db->query("SELECT COUNT(product_id) AS countcode FROM ".DB_PREFIX."edit_code WHERE product_id = '". (int)$product_id ."'");
		if($count_row->row['countcode'] >= 10){
			$query_edit_code = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_code WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified  ");
			$query = $this->db->query("UPDATE " . DB_PREFIX . "edit_code SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', model_product = '". $this->db->escape($model_product) ."', sku_product = '". $this->db->escape($sku_product) ."', upc_product = '". $this->db->escape($upc_product) ."', location = '". $this->db->escape($location_product) ."', tax_class = '". $this->db->escape($tax_class_product) ."', quantity = '". $this->db->escape($quantity_product) ."', minimum = '". $this->db->escape($minimum_product) ."', subtract = '". $this->db->escape($subtract_product) ."',stock_status = '". $this->db->escape($stock_status_product) ."', length_class = '". $this->db->escape($length_class_product) ."',weight = '". $this->db->escape($weight_product) ."',weight_class = '". $this->db->escape($weight_class_product) ."',lwh = '". $this->db->escape($lwh_product) ."', date_modified = NOW() WHERE date_modified = '". $this->db->escape($query_edit_code->rows['0']['date_modified']) ."' ");
		} else {
			$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_code SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', model_product = '". $this->db->escape($model_product) ."', sku_product = '". $this->db->escape($sku_product) ."', upc_product = '". $this->db->escape($upc_product) ."', location = '". $this->db->escape($location_product) ."', tax_class = '". $this->db->escape($tax_class_product) ."', quantity = '". $this->db->escape($quantity_product) ."', minimum = '". $this->db->escape($minimum_product) ."', subtract = '". $this->db->escape($subtract_product) ."', stock_status = '". $this->db->escape($stock_status_product) ."', length_class = '". $this->db->escape($length_class_product) ."',weight = '". $this->db->escape($weight_product) ."', weight_class = '". $this->db->escape($weight_class_product) ."',lwh = '". $this->db->escape($lwh_product) ."', date_modified = NOW()");
		}
	}
	public function ChangeDataProducts($product_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_code WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified DESC");
		return $query->rows;
	}
	
	public function SaveChangeAttributeProduct($product_id, $user_add, $attributes){
		$count_row = $this->db->query("SELECT COUNT(product_id) AS count_attributes FROM ".DB_PREFIX."edit_attributes WHERE product_id = '". (int)$product_id ."'");
			if($count_row->row['count_attributes'] >= 5){
				$query_edit_attributes = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_attributes WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified  ");
				$query = $this->db->query("UPDATE " . DB_PREFIX . "edit_attributes SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', attributes = '". $this->db->escape($attributes) ."', date_modified = NOW() WHERE date_modified = '". $this->db->escape($query_edit_attributes->rows['0']['date_modified']) ."' ");
			} else {
				$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_attributes SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', attributes = '". $this->db->escape($attributes) ."', date_modified = NOW()");
			}
	}
	public function ChangeAttributeProducts($product_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_attributes WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified DESC");
		return $query->rows;
	}
	
	public function SaveChangeoptionProduct($product_id, $user_add, $options_save){
		$count_row = $this->db->query("SELECT COUNT(product_id) AS count_options FROM ".DB_PREFIX."edit_options WHERE product_id = '". (int)$product_id ."'");
		if($count_row->row['count_options'] >= 10){
			$query_edit_attributes = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_options WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified  ");
			$query = $this->db->query("UPDATE " . DB_PREFIX . "edit_options SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', options = '". $this->db->escape($options_save) ."', date_modified = NOW() WHERE date_modified = '". $this->db->escape($query_edit_attributes->rows['0']['date_modified']) ."' ");
		} else {
			$query = $this->db->query("INSERT INTO " . DB_PREFIX . "edit_options SET product_id = '" . (int)$product_id  . "', user_name = '" . $this->db->escape($user_add)  . "', options = '". $this->db->escape($options_save) ."', date_modified = NOW()");
		}
	}
	public function ChangeOptionProducts($product_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "edit_options WHERE product_id = '". (int)$product_id ."' GROUP BY date_modified DESC");
		return $query->rows;
	}
}
?>
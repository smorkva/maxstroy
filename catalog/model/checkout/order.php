...
class ModelCheckoutOrder extends Model {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'multistore_id'           => isset($order_query->row['multistore_id']) ? $order_query->row['multistore_id'] : 0,
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			// Resolve multistore_id from shipping code if not set
			if (empty($order_info['multistore_id']) && strrpos($order_info['shipping_code'], 'multistore_pickup') !== false) {
				$order_info['multistore_id'] = str_replace('multistore_pickup.multistore_', '', $order_info['shipping_code']);
				$this->db->query("UPDATE " . DB_PREFIX . "order SET multistore_id = '" . (int)$order_info['multistore_id'] . "' WHERE order_id = '" . (int)$order_id . "'");
			}
			// Fraud Detection
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				foreach ($order_product_query->rows as $order_product) {
					if (isset($order_info['multistore_id']) && $order_info['multistore_id']) {
						$this->db->query("UPDATE " . DB_PREFIX . "product_to_multistore SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND multistore_id = '".(int)$order_info['multistore_id']."' AND (SELECT `subtract` FROM `" . DB_PREFIX . "product` WHERE product_id = '" . (int)$order_product['product_id'] . "') = '1'");
					}
					...
					foreach ($order_option_query->rows as $option) {
						if (isset($order_info['multistore_id']) && $order_info['multistore_id']) {
							$this->db->query("UPDATE " . DB_PREFIX . "product_option_value_to_multistore SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE product_option_value_id = '" . (int)$option['product_option_value_id'] . "' AND multistore_id = '".(int)$order_info['multistore_id']."' AND (SELECT `subtract` FROM `" . DB_PREFIX . "product_option_value` WHERE `product_option_value_id` = '" . (int)$option['product_option_value_id'] . "') = '1'");
						}
			...
			if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && !in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				foreach($product_query->rows as $product) {
					if (isset($order_info['multistore_id']) && $order_info['multistore_id']) {
						$this->db->query("UPDATE `" . DB_PREFIX . "product_to_multistore` SET quantity = (quantity + " . (int)$product['quantity'] . ") WHERE product_id = '" . (int)$product['product_id'] . "' AND multistore_id = '".(int)$order_info['multistore_id']."' AND (SELECT `subtract` FROM `" . DB_PREFIX . "product` WHERE product_id = '" . (int)$product['product_id'] . "') = '1'");
					}

	...
	public function addLayout($data) {
		...
		if (isset($data['layout_module'])) {
			foreach ($data['layout_module'] as $layout_module) {
				...
				if(!isset($layout_module['tablet_device'])){
					$layout_module['tablet_device'] = 0;
				}
				if(!isset($layout_module['pc_device'])){
					$layout_module['pc_device'] = 0;
				}
			
				$this->db->query("INSERT INTO " . DB_PREFIX . "layout_module SET layout_id = '" . (int)$layout_id . "', code = '" . $this->db->escape($layout_module['code']) . "', position = '" . $this->db->escape($layout_module['position']) . "', mob_device = '" . (int)$layout_module['mob_device'] . "', pc_device = '" . (int)$layout_module['pc_device'] . "', tablet_device = '" . (int)$layout_module['tablet_device'] . "', sort_order = '" . (int)$layout_module['sort_order'] . "'");
			}
		}
